...
foreach ($words as $row) {
	$set = $sqlMap = array();
	$newZhcn = translation(array('tl' => 'zh-CN', 'text' => $row['word_name_zhcn']));
	...
	if ($row['word_name_zhtw']) {
		$newZhtw = translation(array('tl' => 'zh-TW', 'text' => $row['word_name_zhtw']));
		if ($newZhtw != $row['word_name_zhtw']) {
			$set[] = "word_name_zhtw = :word_name_zhtw";
			$sqlMap['word_name_zhtw'] = $newZhtw;
		}
	} else {
		$newZhtw = translation(array('tl' => 'zh-TW', 'text' => $row['word_name_zhcn']));
		$set[] = "word_name_zhtw = :word_name_zhtw";
		$sqlMap['word_name_zhtw'] = $newZhtw;
	}
